...
import de.xoft.scada.config.KafkaProperties;
...
class ScadaKafkaResourceIT {
    ...
    void setup() {
        ...
        ScadaKafkaResource kafkaResource = new ScadaKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/scada-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/scada-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
